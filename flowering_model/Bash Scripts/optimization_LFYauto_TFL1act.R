...
sourceCpp("LFY_auto_TFL1_act.cpp")
...
op_parms <- c(sample(seq(0.01, 10, by = 0.01), 10), sample(seq(1, 4, by = 0.01), 9), sample(seq(0.000001, 1, by = 0.001), 1))
init_parms <- op_parms

low <- c(rep(0.0001, 10), rep(1, 9), 0.00001)
upp <- c(rep(10, 10), rep(4, 9), 1)
...
dat <- data.frame(out)
data_final <- cbind(names(parms_holder), op_parms, dat)
